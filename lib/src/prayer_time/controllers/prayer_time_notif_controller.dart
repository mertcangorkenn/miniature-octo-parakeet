...
class PrayerTimeNotifController extends GetxController {
  ...
  void createPrayerTimeNotif(String prayerTime) {
    ...
    AwesomeNotify.createBasicNotif(
      id: uniqueId,
      title: "Ezan Vakti",
      body: "${prayerTime.capitalize} namaz vakti geldi 🤩",
    ).then((value) {
      if (!value) {
  ...
  void createPrayerTimeReminder({
    ...
    AwesomeNotify.createScheduleNotif(
      id: uniqueId,
      title: "Ezan Vakti Hatırlatıcısı",
      body: minute != 0
          ? "${prayerTime.capitalize} namaz vakti yaklaşıyor ${Duration(seconds: minute).inMinutes}, hazırlanalım 🤩"
          : "${prayerTime.capitalize} namaz vakti geldi, ibadet edelim 🚀",
      dateTime: dateTime,
      hour: hour,
      minute: minute,
    ).then((bool value) {
      if (!value) {
        Get.bottomSheet(
          AppPermissionStatus(
            ...
            message: "Uygulamamız size bildirim göndermek istiyor.",
            onPressed: () {
